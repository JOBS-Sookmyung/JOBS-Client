  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/chat/all/sessions?user_id=" + user.id
      );
      if (!response.ok) {
      ...
      if (!Array.isArray(data)) {
        if (data.session_token && data.session_token === "sessions") {
          console.log(
            "Server returned a single session object instead of an array"
          );
          setSessions([]);
          return;
              ...
              try {
                if (session.session_token) {
                  const chatResponse = await fetch(
                    `http://localhost:8000/chat/${session.session_token}`
                  );
                  if (chatResponse.ok) {
                    const chatData = await chatResponse.json();
                    console.log(
                      `Chat data for session ${session.session_token}:`,
                      chatData
                    );
                    return {
                      ...session,
                      currentQuestion: chatData.messages.filter(
                        (msg) => msg.type === "main_question"
                      ).length,
                      lastMessage:
                        session.last_message || "새로운 면접을 시작해보세요",
                    };
                  }
                }
              } catch (error) {
                console.error(
                  `Error fetching chat data for session ${session.session_token}:`,
                  error
                );
              }
              return session;
