...
const ChatSidebar = ({ selectedSessionToken, handleSelectSession, shouldRefresh, onRefreshComplete ,user}) => {
    ...
    const fetchSessions = async () => {
        try {
            ...
            setLoading(true);
            const response = await fetch("http://localhost:8000/chat/all/sessions?user_id="+user.id);
            if (!response.ok) {
